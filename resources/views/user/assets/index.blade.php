...
<header class="bg-nexus-green p-3 flex items-center gap-4 flex-shrink-0">
    ...
    <h1 class="text-white font-bold text-lg">Aset Saya</h1>
</header>
...
<div class="flex-1 overflow-y-auto p-6">
    ...
    <!-- Page Header -->
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800 mb-2">Aset yang Sedang Anda Pinjam</h2>
        <p class="text-gray-600 text-sm">Kelola aset yang sedang dalam peminjaman aktif Anda</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl p-4 shadow-md border-l-4 border-blue-500">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600">Menunggu Ambil</p>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
                <h3 class="text-lg font-bold text-gray-800">Daftar Aset Aktif</h3>
                <p class="text-sm text-gray-600 mt-1">Aset yang sedang Anda pinjam saat ini</p>
            </div>
            <span class="bg-nexus-green text-white text-xs font-bold px-3 py-1 rounded-full">
                {{ $activeBorrowings->count() }} Aset
            </span>
        </div>
                    ...
                    <div class="flex-1">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-800 truncate">{{ $borrowing->notes }}</p>
                            </div>
                        </div>
                        ...
                        @php
                            ...
                            $progress = $totalDays > 0 ? min(100, max(0, ($usedDays / $totalDays) * 100)) : 0;
                        ...
                        <div class="flex gap-2">
                            @if($borrowing->status === 'approved')
                                <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-blue-700 font-semibold">Silakan ambil aset di ruang inventaris</span>
                                </div>
                            @else
                                <div class="bg-orange-50 border border-orange-200 rounded-lg px-4 py-2 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                    <span class="text-xs text-orange-700 font-semibold">Segera kembalikan sebelum {{ $borrowing->return_date->format('d M Y') }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
        ...
        <div class="text-center py-16">
            ...
            <h3 class="text-xl font-semibold text-gray-700 mb-2">Belum Ada Aset yang Dipinjam</h3>
            <p class="text-gray-500 mb-6 max-w-md mx-auto">
                Anda belum memiliki aset yang sedang aktif dipinjam. Aset yang sudah dikembalikan akan muncul di halaman Riwayat Peminjaman.
            </p>
    ...
    <div class="mt-6 bg-nexus-green rounded-xl p-4 flex items-center gap-3 text-white">
        ...
        <p class="text-sm">Halaman ini hanya menampilkan aset yang sedang aktif. Untuk melihat riwayat peminjaman yang sudah selesai, kunjungi halaman <a href="{{ route('user.borrowing.history') }}" class="underline font-semibold">Riwayat Peminjaman</a>.</p>
    </div>
